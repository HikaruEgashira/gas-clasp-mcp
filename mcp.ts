...
async function checkGitStatus(rootDir?: string): Promise<{ isMainBranch: boolean, hasChanges: boolean }> {
  try {
    const options: { cwd?: string } = {};
    if (rootDir) {
      options.cwd = resolve(rootDir);
...
async function manageClaspJson(rootDir: string, env?: string): Promise<() => Promise<void>> {
  ...
  try {
    ...
    if ((e as NodeJS.ErrnoException).code !== 'ENOENT') {
      await cleanup(); throw new Error(`既存の.clasp.jsonのバックアップに失敗しました: ${e}`);
    }
  ...
  try {
    await fs.copyFile(envClaspJsonPath, claspJsonPath);
     ...
     await cleanup(); throw new Error(`.clasp.${env}.json から .clasp.json へのコピーに失敗しました: ${e}`);
  }

  return cleanup;
}

async function listEnvironments(rootDir: string): Promise<{ environments: string[], currentEnv: string | null }> {
    const files = await fs.readdir(rootDir);
    const envFiles = files.filter(file => file.match(/^\.clasp\..+\.json$/) && file !== '.clasp.json.bak');
    const environments = envFiles.map(file => file.match(/^\.clasp\.(.+)\.json$/)?.[1]).filter((env): env is string => !!env);
    ...
    try {
        ...
        if ((e as NodeJS.ErrnoException).code !== 'ENOENT') {
             console.warn(`Warning: Could not read or parse .clasp.json to determine current environment: ${e}`);
        }
    }
...
const EnvSchema = z.string().optional().describe("操作対象の環境名 (例: dev, prod)。指定しない場合はデフォルトの.clasp.jsonを使用します。");
...
const ClaspSetupArgsSchema = z.object({
  autoInstall: z.boolean().optional().default(true).describe("claspがインストールされていない場合、自動的にインストールするかどうか"),
  autoLogin: z.boolean().optional().default(false).describe("自動的にGoogleアカウントへのログインを開始するかどうか"),
  global: z.boolean().optional().default(true).describe("インストールする場合、グローバルインストールするかどうか"),
  listProjects: z.boolean().optional().default(true).describe("セットアップ後にプロジェクト一覧を表示するかどうか"),
});
...
const ClaspCreateArgsSchema = z.object({
  ...
  type: z.enum(["standalone", "docs", "sheets", "slides", "forms", "webapp", "api"]).optional().describe("プロジェクトタイプ"),
  env: EnvSchema.describe("作成する環境名。指定した場合、成功後に .clasp.{env}.json が保存されます。"),
});

const ClaspCloneArgsSchema = z.object({
  scriptId: z.string().describe("クローンするスクリプトID"),
  rootDir: z.string().describe("クローン先のディレクトリ"),
   env: EnvSchema.describe("クローンする環境名。指定した場合、成功後に .clasp.{env}.json が保存されます。"),
});

const ClaspPullArgsSchema = z.object({
  rootDir: z.string().describe("プルするプロジェクトのディレクトリ"),
  scriptId: z.string().optional().describe("プルするスクリプトID（指定しない場合は.clasp.jsonから取得）"),
   env: EnvSchema,
});
...
const TOOLS: Tool[] = [
  {
    name: "clasp_setup",
    description: "claspの環境を一括セットアップします。インストール状態をチェックし、必要に応じてインストールします。オプションでGoogleアカウントへのログインも行います。",
...
server.setRequestHandler(CallToolRequestSchema, async (req: any) => {
  ...
  switch (name) {
    case "clasp_setup": {
      ...
      resultText += "\n\n"
      resultText += "注意: GASの操作はMCPツールを利用してください。手動で \`npm install -g @google/clasp\` を実行したり、\`npx @google/clasp\`のようにterminalでclaspを実行しないようにしましょう。"
    ...
    case "clasp_clone": {
      ...
      try {
        ...
        await executeCommand([`[ -d "${rootDir}" ] && echo "exists" || echo "not exists"`])
          .then(output => { directoryExists = output.trim() === "exists"; });
        
        
        await executeCommand([`[ -f "${rootDir}/.clasp.json" ] && echo "exists" || echo "not exists"`])
          .then(output => { hasClaspConfig = output.trim() === "exists"; });
      } catch (error) {
        ...
        try {
          if (env === "production") {
            ...
            if (!gitStatus.isMainBranch) {
              throw new Error("本番環境(production)へのプッシュは、mainまたはmasterブランチからのみ可能です。");
            }
            
            if (gitStatus.hasChanges) {
              throw new Error("本番環境(production)へのプッシュ前に、すべての変更をコミットする必要があります。");
            }
          }
      ...
      try {
        ...
        try {
          if (env === "production") {
            ...
            if (!gitStatus.isMainBranch) {
              throw new Error("本番環境(production)へのデプロイは、mainまたはmasterブランチからのみ可能です。");
            }
            
            if (gitStatus.hasChanges) {
              throw new Error("本番環境(production)へのデプロイ前に、すべての変更をコミットする必要があります。");
            }
          }
    ...
    case "clasp_list_envs": {
      ...
      try {
        ...
        resultText += "\n使用方法:\n";
        resultText += "各環境を利用するには、clasp_pull, clasp_push, clasp_deploy などのコマンドで env パラメータを指定します。\n";
